...
def negativity_qutip(rho, subsys, method='tracenorm'):
    """
...
def negativity_hor(rho,subsys):
    '''
    Computa la negatividad como la suma de los autovalores negativos de la matriz densidad multipartita.
    -Args:
        rho:matriz densidad (Qobj)
        subsys: lista binaria o de booleanos indicando sobre que subsistemas se realiza la partial_transpose() (ver docs de Qutip sobre este argumento)
    -Returns:
        N: Negatividad del estado. Suma de los autovalores negativos'''
    if rho.type == 'ket' or rho.type == 'bra':
        rho=ket2dm(rho)
    else:
        None
    eigenvals=partial_transpose(rho,subsys).eigenenergies()
    N=np.abs(np.sum(eigenvals[eigenvals <0]))
    return N

def tangle3(psi):
    """Calculates the 3tangle """
...
def entropy_of_entanglement_2xN(psi,N_c):
    """
...
def roof_etanglement_bipartite(rank,V,initial_point,N_c,dims=0,min_step=1e-10):
    ...
    def cost(U):
        ...
        for i in range(rank):
            ...
            if dims==0:
                ent = entropy_of_entanglement_2xN(psi_i,N_c)
            elif dims==1:
...
#     np.savetxt(f'CRE data/{new_number} resultcost.txt',[resultcost_01,resultcost_02,resultcost_12],header='E_01; E_02; E_12')
