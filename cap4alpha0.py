...
ax_pob_ABC.plot(t/T,[np.abs(sol_d.states[i][9][9]) for i in range(steps)],color='black',label=ops_name[9]) #gg0


ax_pob_ABC.plot(t/T,[sol_d.states[i][3][3] for i in range(steps)],color=(0, 255/255, 0),label=ops_name[3]) #eg0
ax_pob_ABC.plot(t/T,[sol_d.states[i][6][6] for i in range(steps)],color=(255/255, 0, 0),label=ops_name[6]) #ge0
ax_pob_ABC.plot(t/T,[sol_d.states[i][10][10] for i in range(steps)],color=(0, 0, 255/255),label=ops_name[10]) #gg1
...
fig_pob_ABC_new = plt.figure(figsize=(8,6))
ax_pob_ABC_new = fig_pob_ABC_new.add_subplot()
ax_pob_ABC_new.set_title('ABC new')
ax_pob_ABC_new.set_xlabel('$t/T$')
ax_pob_ABC_new.set_ylabel('Amp. Prob. ')
ax_pob_ABC_new.set_ylim(0,1)
ax_pob_ABC_new.set_xlim(0,t_final/T)
# for i,ops_name_i in enumerate(ops_name):
#     ax_pob_ABC.plot(t/T,ops_expect_d[ops_name_i],color=inferno(12)[i])#,label=ops_name_i)
ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][9][9]) for i in range(steps)],color='black',label='gg0') #gg0

ax_pob_ABC_new.plot(t/T,[sol_d.states[i][3][3]/2+sol_d.states[i][6][6]/2+np.real(sol_d.states[i][3][6]) for i in range(steps)],color=(0, 255/255, 0),label='eg0+') #eg0+ge0
ax_pob_ABC_new.plot(t/T,[sol_d.states[i][3][3]/2+sol_d.states[i][6][6]/2-np.real(sol_d.states[i][3][6]) for i in range(steps)],color=(255/255, 0, 0),label='eg0-') #eg0-ge0
ax_pob_ABC_new.plot(t/T,[sol_d.states[i][10][10] for i in range(steps)],color=(0, 0, 255/255),label='gg1') #gg1

ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][3][9]+sol_d.states[i][6][9])/np.sqrt(2) for i in range(steps)],linestyle='dashed',color=(28/255, 176/255, 28/255)) #Coherenencia gg0/eg0+ge0
ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][3][9]-sol_d.states[i][6][9])/np.sqrt(2) for i in range(steps)],linestyle='dashed',color=(186/255, 23/255, 23/255)) #Coherenencia gg0/eg0-ge0

ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][9][10]) for i in range(steps)],linestyle='dashed',color=(0, 0, 170/255))

ax_pob_ABC_new.plot(t/T,[0.5*(sol_d.states[i][3][3]-2*np.imag(sol_d.states[i][3][6])-sol_d.states[i][6][6]) for i in range(steps)],linestyle='dashed',color=(255/255, 209/255, 0)) #Coherencia eg0+/eg0-

ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][3][10]+sol_d.states[i][3][10])/np.sqrt(2) for i in range(steps)],linestyle='dashed',color=(0, 209/255, 210/255)) #Coherenencia gg1/eg0-ge0
ax_pob_ABC_new.plot(t/T,[np.abs(sol_d.states[i][3][10]-sol_d.states[i][3][10])/np.sqrt(2) for i in range(steps)],linestyle='dashed',color=(235/255, 18/255, 235/255)) #Coherenencia gg1/eg0-ge0
# ax_pob_ABC.plot(t/T,np.array(ops_expect_d['eg0'])+np.array(ops_expect_d['ee0']),label='$e_A0_C$',color='red',marker=',',alpha=0.5) 
# ax_pob_ABC.plot(t/T,np.array(ops_expect_d['gg1'])+np.array(ops_expect_d['ge1']),label='$g_A1_C$',color='blue',marker=',',alpha=0.5)

ax_pob_ABC_new.legend(loc='upper right')
fig_pob_ABC_new.savefig(f'./graficos/{prefijo} {psi0Name} abc new.png')
